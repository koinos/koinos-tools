...
int main( int argc, char** argv )
{
  try
  {
    ...
    options.add_options()( HELP_OPTION "," HELP_FLAG, "print usage message" )(
      PRIVATE_KEY_OPTION "," PRIVATE_KEY_FLAG,
      boost::program_options::value< std::string >()->default_value( "private.key" ),
      "private key file" )( INPUT_OPTION "," INPUT_FLAG,
                            boost::program_options::value< std::string >()->default_value( "" ),
                            "input to use to generate the random proof (base64 encoded)" );
    ...
    std::string json_str = "{ \"proof\": \"" + util::to_base64< std::string >( proof ) + "\", \"proof_hash\": \""
                           + util::to_base64< std::string >( util::converter::as< std::string >( proof_hash ) )
                           + "\" }";
